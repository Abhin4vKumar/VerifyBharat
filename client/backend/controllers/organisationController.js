...
exports.addEmployee = catchAsyncErrors(async(req,res,next)=>{
    ...
    const user_ = await User.findByIdAndUpdate(user._id , {employee : true , workOrganisation : org._id} ,{
        new: true,
        runValidators: true,
        useFindAndModify: false,
    } );
...
exports.removeEmployee = catchAsyncErrors(async(req,res,next)=>{
    ...
    await org.save();
    const user_ = await User.findByIdAndUpdate(user._id , {employee : false , workOrganisation : null} ,{
        new: true,
        runValidators: true,
        useFindAndModify: false,
    } );
